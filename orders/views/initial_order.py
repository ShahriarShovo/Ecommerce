...
def initial_order(request):
    ...
    if request.method == "POST" or "post":
        ...
        if address:
          BillingAddress.objects.update_user_billing_address(request.user,zip,phone,house_number,street,state,country)
        else:
            BillingAddress.objects.create_user_billing_address(request.user, zip,phone,house_number,street,state,country)
